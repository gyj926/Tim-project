...
<body>
	...
	<div class="container">
		...
		<h3 class="title"> 현재 상영작 [ 상영예정작 수 : ${moviePageDto.totalCount}] </h3>
		
		<div class="searchTop">
			<div class="frm1">
				<form action="/movie/currentMovie" method="get">
		...
		<div class="row product">
			<c:choose>
							...
							<div class="thumbnail thumbnailBox">
								...
								<div class="caption">
									...
									<div class="thumbnailBtns">
										...
										<a href="/reservation/?movieId=${movie.movieId }" class="thumbnailBtn">예매하기</a>
									</div>
								</div>
							</div>
						</div>
				...
				<c:otherwise>
					<div class="emptyMovie" align="center">
						<div><h1 ><img src="/images/검색없음.png"  ><br><br>조회된 영화가 없습니다.</h1></div>
					</div>
				</c:otherwise>
			</c:choose>	
		</div>
		...
		<div id="page_control">
			<c:if test="${ moviePageDto.totalCount gt 0 }">
				...
				<c:if test="${ moviePageDto.startPage gt moviePageDto.pageBlock }">
					<a href="/movie/currentMovie?pageNum=${ moviePageDto.startPage - moviePageDto.pageBlock }&category=${ pageDto.category }">[이전]</a>
				</c:if>
				
				<%-- 페이지블록 내에서의 시작페이지부터 끝페이지까지 번호출력 --%>
				<c:forEach var="i" begin="${ moviePageDto.startPage }" end="${ moviePageDto.endPage }" step="1">
					<a href="/movie/currentMovie?pageNum=${ i }&category=${ moviePageDto.category }">
						<c:choose>
				...
				<c:if test="${ moviePageDto.endPage lt moviePageDto.pageCount }">
					<a href="/movie/currentMovie?pageNum=${ moviePageDto.startPage + moviePageDto.pageBlock }&category=${ pageDto.category }">[다음]</a>
				</c:if>
			</c:if>
		</div>
	</div>	<!-- container -->
